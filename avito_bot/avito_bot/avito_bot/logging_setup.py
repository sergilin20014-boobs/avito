...
FILE_FORMAT = "%(asctime)s | %(levelname)-8s | %(short_name)-12s | %(message)s"
CONSOLE_FORMAT = "%(asctime)s | %(levelname_colored)s | %(short_name)-12s | %(message)s"
...
class BotFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        record.short_name = _short_module(record.name)
